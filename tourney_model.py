...
history_X = np.array(history[['Seed Adv.', 'AdjEM Adv.', 'NCSOS Adv.', 'KenPom', 'FiveThirtyEight']])
history_y = np.array(history['Survive'])

X_train, X_test, y_train, y_test = train_test_split(history_X, history_y, random_state=18)
clf = RandomForestClassifier(random_state=47, n_estimators=1000)
...
this_year_X = np.array(this_year[['Seed Adv.', 'AdjEM Adv.', 'NCSOS Adv.', 'KenPom', 'FiveThirtyEight']])
